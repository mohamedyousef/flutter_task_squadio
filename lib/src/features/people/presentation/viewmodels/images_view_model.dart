...
class ImagesViewModel extends PaginationController<int, ImageModel> {
  final int _personId;
  final PeopleUseCases _peopleUseCases;

  ImagesViewModel(this._personId, this._peopleUseCases);
  ...
  Future<void> fetchPage(int? pageKey) async {
    try {
      final response = await _peopleUseCases.fetchPersonImages(
        _personId,
      );
      response.when(
        success: (data) {
